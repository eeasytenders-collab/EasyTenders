            ...
            <View className="px-4 pb-3">
                <View className="flex-row items-center py-5">
                    <Pressable
                        ...
                        android_ripple={{ color: colors.blueDark }}
                    >
                        <Image
                ...
                <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
                    <View className="px-4 pt-5 pb-24">
                        ...
                        <View className="flex-row items-center justify-between mb-4">
                            ...
                            <Pressable>
                                <Text className="text-primary dark:text-blue-300">See All</Text>
                            </Pressable>
                        </View>
                                ...
                                {CITIES.map((c) => {
                                    ...
                                    return (
                                        <Pressable
                                            ...
                                            className={`px-4 h-12 rounded-xl border items-center justify-center ${active ? 'bg-primary border-primary' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-600'}`}
                                        >
                        ...
                        <View className="px-1">
                            <Slider
                                ...
                                step={10000}
                                minimumTrackTintColor={isDark ? colors.blueLight : colors.primary}
                                maximumTrackTintColor={isDark ? colors.border : colors.cardBg}
                                thumbTintColor={isDark ? colors.blueLight : colors.primary}
                                style={{ height: 40 }}
                            />
                            <View className="flex-row justify-between mt-1">
                                <Text className="text-slate-500 dark:text-slate-400">Rs. {minBudget.toLocaleString('en-IN')}</Text>
                                <Text className="font-semibold text-primary dark:text-blue-300">Rs. {Math.round(budget).toLocaleString('en-IN')}</Text>
                            </View>
                        </View>
                            ...
                            {CATEGORIES.map((c) => {
                                ...
                                return (
                                    <Pressable
                                        key={c}
                                        onPress={() => setCat(c)}
                                        className={`px-5 h-11 rounded-xl border items-center justify-center ${active ? 'bg-primary border-primary' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-600'}`}
                                    >
                                        <Text className={active ? 'text-white' : 'text-slate-600 dark:text-slate-300'}>{c}</Text>
                                    </Pressable>
                                );
                            ...
                            {visibleSubcats.map((label) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <View
                                            className={`h-6 w-6 rounded-full border-2 items-center justify-center ${active ? 'border-blueLight' : 'border-slate-400 dark:border-slate-500'}`}
                                        >
                                            {active && <View className="h-3.5 w-3.5 rounded-full bg-blueLight" />}
                                        </View>
                                    </Pressable>
                                );
                            })}
                        ...
                        <View className="mt-8">
                            ...
                            <Pressable onPress={reset} className="mt-4 items-center">
                                <Text className="text-base text-primary dark:text-blue-300">Reset Filter</Text>
                            </Pressable>
                        </View>
                    </View>
                </ScrollView>
